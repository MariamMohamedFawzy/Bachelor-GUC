	...
	public static ArrayList<Review> getReviewsByCategoryAndLocation(
		...
		if (factory != null) {
			...
			try {
				...
				String hql3 = "From json.Review WHERE (stars = 5 or stars = 1) AND businessId IN :list_ids";
...
//				double[][] theta = model.theta;
				...
				for (int i = 0; i < docsIndex.size(); i++) {
					...
					int[] z = GUIFunctions.getModel().z[docsIndex.get(i)];
					...
					for (String line : docLines) {
						...
						for (int j = 0; j < words.size(); j++) {
							...
							if (Stopwords.isStopword(words.get(j))
								...
								if (z[k] == topicNum) {
									if (words.get(j).equals("GOODREVIEW")) {
										temp += 1;	
									}
									else {
										temp -= 1;
